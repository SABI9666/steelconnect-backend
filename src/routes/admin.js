...
// Helper: Auto-detect chart types and build dashboard config from parsed data
function autoGenerateDashboardConfig(sheets, frequency) {
    const configs = [];
    for (const [sheetName, rows] of Object.entries(sheets)) {
        if (rows.length === 0) continue;
        const headers = Object.keys(rows[0]);
        // Find date/label column and numeric columns
        let labelCol = null;
        const numericCols = [];
...
// POST /api/admin/dashboards/upload - Upload spreadsheet, parse, auto-generate dashboard
router.post('/dashboards/upload', upload.single('spreadsheet'), async (req, res) => {
    try {
        if (!req.file) {
            return res.status(400).json({ success: false, message: 'Please upload a spreadsheet file (.xlsx, .xls, .csv)' });
        }
        const { contractorEmail, contractorName, frequency, title, description } = req.body;
        if (!contractorEmail) {
            return res.status(400).json({ success: false, message: 'Contractor email is required' });
        }

        // Parse spreadsheet
        const sheets = parseSpreadsheet(req.file.buffer, req.file.originalname);
        ...
        const dashboardCharts = autoGenerateDashboardConfig(sheets, frequency || 'daily');
        ...
        // Store raw data and config in Firestore
        const dashboardData = {
            title: title || `Dashboard - ${req.file.originalname}`,
        ...
        console.log(`[DASHBOARD] Created dashboard ${docRef.id} for ${contractorEmail} from ${req.file.originalname}`);

        res.json({
            success: true,
            message: 'Dashboard auto-generated from spreadsheet',
        ...
        res.status(500).json({ success: false, message: 'Failed to process spreadsheet' });
    }
});
