...
<p style="margin:20px 0;"><a href="https://steelconnectapp.com/?section=my-estimations&estimationId=${estimation._id || estimation.id}" ${S.btn}>View Estimation Result</a></p>
...
// ============================================================
// AI ESTIMATION COMPLETED NOTIFICATION
// ============================================================
export async function sendAIEstimationCompletedEmail(contractor, estimation) {
    try {
        const projectName = estimation.projectTitle || estimation.projectName;
        const estimatedAmount = estimation.estimatedAmount
            ? `$${Number(estimation.estimatedAmount).toLocaleString()}`
            : 'See report';
        const htmlContent = `
<h2 ${S.h2}>Your AI Estimation is Ready</h2>
<p ${S.p}>Hi ${contractor.name},</p>
<p ${S.p}>Great news! The AI cost estimation for your project has been completed and is ready to view.</p>
<table ${S.table}>
<tr><td ${S.tdLabel}>Project</td><td ${S.tdValue}>${projectName}</td></tr>
<tr><td ${S.tdLabel}>Estimated Cost</td><td ${S.tdValue}>${estimatedAmount}</td></tr>
<tr><td ${S.tdLabel}>Completed</td><td ${S.tdValue}>${new Date().toLocaleDateString()}</td></tr>
</table>
<div ${S.success}><strong>AI Report Ready</strong> — Your detailed cost breakdown is available in the portal.</div>
<p style="margin:20px 0;"><a href="https://steelconnectapp.com/?section=my-estimations&estimationId=${estimation.id}" ${S.btn}>View AI Report</a></p>
<p ${S.muted}>This estimate was generated by our AI engine. For questions, reply to this email.</p>`;

        return await sendEmail({
            to: contractor.email,
            subject: `AI Estimation Complete — "${projectName}"`,
            htmlContent,
        });
    } catch (error) {
        console.error('Error sending AI estimation completed email:', error);
        throw error;
    }
}
...
export default {
    ...
    sendAIEstimationCompletedEmail,
