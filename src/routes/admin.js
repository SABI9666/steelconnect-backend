...
import { fetchGoogleSheetData, parseSpreadsheet as parseSpreadsheetUtil, autoGenerateDashboardConfig as autoGenerateUtil } from '../utils/sheetAnalyzer.js';
...
router.post('/dashboards/upload', upload.single('spreadsheet'), async (req, res) => {
    try {
        ...
        if (hasFile) {
            ...
            try {
                console.log(`[DASHBOARD] Fetching Google Sheet data from: ${googleSheetUrl.trim()}`);
                ...
                dashboardCharts = autoGenerateUtil(sheets, frequency || 'daily');
                console.log(`[DASHBOARD] Auto-generated ${dashboardCharts.length} charts from Google Sheet`);

                if (dashboardCharts.length === 0) {
                    return res.status(400).json({ success: false, message: 'Could not extract numeric data from the Google Sheet. Ensure it contains columns with numeric values.' });
                }
            } catch (sheetError) {
                console.error('[DASHBOARD] Google Sheet fetch error:', sheetError.message);
