...
router.post('/upload-sheet', upload.single('spreadsheet'), async (req, res) => {
    try {
        ...
        const { title, frequency, description, dataType } = req.body;
        ...
        console.log(`[ANALYSIS] Contractor ${userEmail} uploaded sheet -> Dashboard ${docRef.id} created (pending approval)`);

        res.json({
            success: true,
            message: 'Your data has been uploaded and a dashboard has been auto-generated. It is now pending admin approval.',
        ...
        res.status(500).json({ success: false, message: 'Failed to process your spreadsheet. Please try again.' });
    }
});
