...
router.post('/dashboard/:id/upload-data', (req, res, next) => {
    ...
    try {
        ...
        if (hasLink) {
            ...
            // For HTML template reports, fetch sheet data and store as templateData
            // so the HTML report auto-updates with the new data
            ...
            if (data.reportType === 'html') {
                try {
                    ...
                    if (result && result.sheets) {
                        ...
                        updateData.templateData = templateData;
                        updateData.templateDataUpdatedAt = new Date().toISOString();
                        updateData.sheetNames = Object.keys(result.sheets);
                        sheetsProcessed = Object.keys(result.sheets).length;
                    }
                } catch (e) {
                    console.error('[ANALYSIS] Failed to fetch sheet data from link for template:', e.message);
                }
            }

            // For auto-generated chart dashboards, also update charts from linked data
            if (data.reportType !== 'html' && data.reportType !== 'pdf') {
                try {
                    const result = await fetchSheetData(trimmedUrl);
                    if (result && result.sheets) {
                        const sheets = result.sheets;
                        const dashboardCharts = autoGenerateDashboardConfig(sheets, data.frequency || 'daily');
                        updateData.charts = createLightweightCharts(dashboardCharts, 100);
                        updateData.totalChartsGenerated = dashboardCharts.length;
                        updateData.sheetNames = Object.keys(sheets);
                        sheetsProcessed = Object.keys(sheets).length;
                        try {
                            const pa = generatePredictiveAnalysis(sheets, dashboardCharts);
                            updateData.predictiveAnalysis = {
                                forecasts: (pa.forecasts || []).slice(0, 5),
                    ...
                    console.error('[ANALYSIS] Failed to fetch/process sheet data from link:', e.message);
                }
            }
            ...
            const isHtmlTemplate = data.reportType === 'html';
            console.log(`[ANALYSIS] User ${userEmail} updated sheet link for dashboard ${req.params.id} (${isHtmlTemplate ? 'HTML auto-update' : 'link update'})`);
            return res.json({
                success: true,
                message: isHtmlTemplate
                    ? `Sheet link updated! Your HTML report has been auto-updated with ${sheetsProcessed} sheet(s).`
                    : `Sheet link updated! Dashboard will auto-refresh with latest data.`,
                sheetsProcessed,
                autoUpdated: isHtmlTemplate
            });
        }
