...
import { fetchGoogleSheetData, parseSpreadsheet, autoGenerateDashboardConfig } from '../utils/sheetAnalyzer.js';
...
router.post('/upload-sheet', upload.single('spreadsheet'), async (req, res) => {
    try {
        ...
        if (hasFile) {
            ...
            // Fetch data from Google Sheet URL and auto-generate dashboard
            try {
                console.log(`[ANALYSIS] Fetching Google Sheet data from: ${googleSheetUrl.trim()}`);
                ...
                dashboardCharts = autoGenerateDashboardConfig(sheets, frequency || 'daily');
                console.log(`[ANALYSIS] Auto-generated ${dashboardCharts.length} charts from Google Sheet`);

                if (dashboardCharts.length === 0) {
                    return res.status(400).json({ success: false, message: 'Could not extract numeric data from the Google Sheet. Ensure the sheet contains columns with numeric values.' });
                }
            } catch (sheetError) {
                console.error('[ANALYSIS] Google Sheet fetch error:', sheetError.message);
        ...
        const message = `Dashboard auto-generated with ${dashboardCharts.length} charts from ${hasFile ? 'uploaded file' : 'Google Sheet'}! Sent to admin for approval.`;
