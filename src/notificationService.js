...
export class NotificationService {
    static async createNotification(userId, type, message, metadata = {}) {
        try {
            const notification = {
                userId,
                type, // 'job', 'quote', 'message', 'user', 'file'
                message,
            ...
            // Here you could integrate WebSocket/Socket.IO for real-time delivery
    ...
    static async notifyJobCreated(jobData) {
        try {
            // Notify all designers about new job opportunity
    ...
    static async notifyJobStatusChanged(jobData, oldStatus, newStatus) {
        try {
            ...
            // Notify job poster about status changes
            if (jobData.posterId) {
    ...
    static async notifyQuoteSubmitted(quoteData, jobData) {
        try {
            // Notify job poster about new quote
            if (jobData.posterId) {
    ...
    static async notifyQuoteStatusChanged(quoteData, jobData, newStatus) {
        try {
            // Notify quote submitter about status change
        ...
        try {
            ...
            if (recipientIds.length > 0) {
                await this.createNotificationForMultipleUsers(
                    recipientIds,
                    'message',
                    `New message from ${messageData.senderName}: "${messageData.text.substring(0, 50)}${messageData.text.length > 50 ? '...' : ''}"`,
                    { 
    ...
    static async notifyEstimationStatusChanged(estimationData, newStatus) {
        try {
            ...
            // Get contractor user ID from email
...
// src/routes/notifications.js - ENHANCED VERSION
